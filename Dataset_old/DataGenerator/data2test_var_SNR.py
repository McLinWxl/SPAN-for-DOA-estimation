...
fs = 4 * f0
C = M * (M - 1)
SNR_list = np.arange(-12, 13, 1)

D_start = -60
D_stop = 60

DOA_test = np.zeros((len(SNR_list), times, 2))
for idx, k in itertools.product(range(len(SNR_list)), range(times)):
...
for idx in track(range(len(SNR_list)), description="Generating data"):
    for i in range(DOA_test.shape[1]):
        ...
        temp1 = add_awgn(X1, X1_power, SNR_list[idx])
        temp2 = add_awgn(X2, X2_power, SNR_list[idx])
        X[idx, i] = temp1 + temp2
...
# save as h5 file
with h5py.File('../Data/TestData_varSNR_16.h5', 'w') as f:
