...
from DataGenerator import signal_generate, feature_extract_R, add_awgn

M = 8
snapshot_list = np.arange(50, 401, 10)
f0 = 1e6
fc = 1e6
lam = fc / f0
fs = 4 * f0
...
DOA_test = np.zeros((len(snapshot_list), times, 2))
for idx, k in itertools.product(range(len(snapshot_list)), range(times)):
    DOA_test[idx, k, 0] = -8 + 1 * np.random.uniform(-0.5, 0.5)
    DOA_test[idx, k, 1] = 8 + 1 * np.random.uniform(-0.5, 0.5)
...
for idx in track(range(len(snapshot_list)), description="Generating data"):
    Signal_wave = np.zeros((DOA_test.shape[0], DOA_test.shape[1], 2, snapshot_list[idx])) + 1j * np.zeros(
        (DOA_test.shape[0], DOA_test.shape[1], 2, snapshot_list[idx]))
    X = np.zeros((DOA_test.shape[0], DOA_test.shape[1], M, snapshot_list[idx])) + 1j * np.zeros(
        (DOA_test.shape[0], DOA_test.shape[1], M, snapshot_list[idx]))

    for i in range(DOA_test.shape[1]):
        X1, _, s1 = signal_generate(M, snapshot_list[idx], DOA_test[idx, i, 0])
        X2, _, s2 = signal_generate(M, snapshot_list[idx], DOA_test[idx, i, 1])
        X1_power = (np.matmul(s1, s1.conj().T)).real / snapshot_list[idx]
        X2_power = (np.matmul(s2, s2.conj().T)).real / snapshot_list[idx]
...
with h5py.File('../Data/TestData_varSnapshots_16.h5', 'w') as f:
