...
angle_separation = np.arange(2, 20.1, 1)
times = 1000
DOA_test = np.zeros((len(angle_separation), times, 2))
for idx, k in itertools.product(range(len(angle_separation)), range(times)):
    DOA_test[idx, k, 0] = -0.5 * angle_separation[idx] + np.random.uniform(-0.5, 0.5)
    DOA_test[idx, k, 1] = 0.5 * angle_separation[idx] + np.random.uniform(-0.5, 0.5)
...
for idx in track(range(len(angle_separation)), description="Generating data"):
    ...
    for i in range(DOA_test.shape[1]):
        X1, _, s1 = signal_generate(M, snapshot, DOA_test[idx, i, 0])
        X2, _, s2 = signal_generate(M, snapshot, DOA_test[idx, i, 1])
...
with h5py.File('../Data/TestData_Separation_Ori.h5', 'w') as f:
