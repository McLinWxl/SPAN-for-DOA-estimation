...
# k1 = np.array([5.5, 13.5, 20.7]) # LOW
# k1 = np.array([50, 60, 70]) # HIGH
# k1 = np.array([5.5, 13.5, 20.67, 50, 60, 70]) # ALL
k1 = np.array([5.5, 13.5, 20.5])
k = np.tile(k1, 1)
for l in range(len(k)):
    DOA1 = np.arange(D_start + 10, D_stop - 10 - k[l], 1)
    DOA2 = np.arange(D_start + 10 + k[l], D_stop - 10, 1)
    DOA11 = np.concatenate((DOA11, DOA1))
    DOA22 = np.concatenate((DOA22, DOA2))
DOA_train = np.vstack((DOA11, DOA22)) + np.random.uniform(-0.5, 0.5, len(DOA11))
...
with h5py.File('../Data/DataTest.h5', 'w') as f:
