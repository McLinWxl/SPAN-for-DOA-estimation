...
class LISTA(torch.nn.Module):
    ...
    def __init__(self, dictionary, **kwargs):
        ...
        self.Wg = torch.nn.Parameter(torch.randn([self.num_layers, self.num_meshes, self.num_meshes]) +
                                     1j * torch.randn([self.num_layers, self.num_meshes, self.num_meshes]), requires_grad=True)
...
class AMI_LISTA(torch.nn.Module):
    ...
    def forward(self, covariance_vector: torch.Tensor):
        ...
        for layer in range(self.num_layers):
            identity_matrix = (torch.eye(self.num_meshes) + 1j * torch.zeros([self.num_meshes, self.num_meshes])).to(self.device)
            if not self.mode:
                W1 = self.W1[layer]
                W2 = self.W2[layer]
            elif self.mode == 'tied':
